	...
	init: function( editor ) {
		editor.addCommand('simage', {
			exec: function (editor) {
				...
				a.setAttribute('accept', '.jpg,.bmp,.jpeg,.png,.tif,.gif,.svg')
				a.click()
				a.onchange = function(){
					file = a.files[0];
					$(CKEDITOR.instances.workflow).trigger('imageUploading')
					curr = CKEDITOR.instances.workflow
					if (file.size > 12000000){
						b = document.createElement('div')
						b.className = 'message alert alert-danger'
						m = document.createElement('span')
						m.innerHTML = "Image size exceeded! Please upload image of less than 12 MB."
						b.appendChild(m)
						...
						$(CKEDITOR.instances.workflow).trigger('imageUploaded')
						return
					...
					loaderHtmlStr = '<div style="position: relative; z-index: 100;width: 100%;height: 100%;text-align: center;background: white;opacity: 0.75;pointer-events:none">' + '</div>'
					...
					CKEDITOR.instances.workflow.setReadOnly(true)
					$.ajax({
						...
						success: function(data, textStatus, jqXHR) {
							var isNew;
							CKEDITOR.instances.workflow.setReadOnly(false)
